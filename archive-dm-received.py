...
tweetfile = tweetdir + 'dmessages-received.txt'
...
# Get the ID of the last downloaded message.
a = {}
...
tweets = api.direct_messages(since_id=lastID)

# Write them out to the dmessages-received.txt file.
with open(tweetfile, 'a') as f:
    for t in reversed(tweets):
      ...
      lines = ['',
               t.text,
               '',
               'at ' + ts.strftime(datefmt) + ' from [@' + t.sender.screen_name + '](' + urlprefix + t.sender.screen_name + ')',
               '- - - - -',
               '']
